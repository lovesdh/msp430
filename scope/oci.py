...
def real_time_plotting():
    global plotting, time_values, voltage_values_max, voltage_values_min, voltage_values_ave
    ...
    while plotting:
        ...
        if data:  # 确保读取到数据            
            ...
            # if len(voltage_values_max) >= 2:
            #     new_x = np.array([len(voltage_values_max) - 2, len(voltage_values_max) - 1])
            #     new_y = np.array([voltage_values_max[-2], voltage_values_max[-1]])
            #     new_x_interp = np.linspace(new_x[0], new_x[1], 50)
            #     f_max = interp1d(new_x, new_y, kind='linear', fill_value="extrapolate")
            #     new_y_interp = f_max(new_x_interp)

            #     prev_x_max.extend(new_x_interp)
            #     prev_y_max.extend(new_y_interp)

            #     ax1.plot(prev_x_max, prev_y_max, '-', color='red', label='Max Voltage')
            # else:
            #     ax1.plot(len(voltage_values_max) - 1, voltage_values_max[-1], 'ro', label='Max Voltage')
