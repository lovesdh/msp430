...
# 初始化串行端口
try:
    ser = serial.Serial(port="COM4", bytesize=8, baudrate=9600, stopbits=1, timeout=1)
...
# # 初始化最小电压值
# min_voltage = 0  # 初始化为无穷大，确保任何电压值都会更小
...
def real_time_plotting():
    global plotting, time_values, voltage_values
    last_data_time = time.time()    
    min_voltage = float('inf')  # 在函数内部初始化min_voltage为正无穷大
    ...
    while plotting:
        ...
        if data:  # 确保读取到数据
            ...
            # 使用正则表达式提取电压值
            voltage_pattern = re.compile(r'相应电压值为：(\d+\.\d+)')
            if voltage_pattern.search(data):
                voltage_value = float(voltage_pattern.search(data).group(1))
                if voltage_value < min_voltage:  # 更新最小电压值
            ...
            # 应用移动平均滤波器
            if len(voltage_values) >= 10:  # 确保数据点足够多以应用滤波器
                filtered_voltage = moving_average_filter(voltage_values, window_size=10)
                # 确保time_values列表与filtered_voltage长度匹配
                time_values = time_values[-len(filtered_voltage):]  # 截取最后10个数据点
            else:
                filtered_voltage = voltage_values  # 如果列表为空，返回空列表
            
            # # 限制time_values和voltage_values的长度
            # if len(time_values) > int(refresh_rate * time_window):
            #     time_values = time_values[-int(refresh_rate * time_window):]
            #     voltage_values = voltage_values[-int(refresh_rate * time_window):]
            ...
            ax.set_title('oscilloscope')
            filtered_voltage = moving_average_filter(voltage_values, window_size=10)
            ax.plot(time_values, voltage_values, 'o-', color='blue',markersize=3, linewidth=1,label='Original Signals')
            ax.plot(time_values,filtered_voltage,'o-', color='red',markersize=5, linewidth=2,label='Filtered Signals')

            # 添加文本注释
            # ax.text(0.5, 1.15, '这是正常波动范围', fontsize=12, color='green')
            # 添加带箭头的注释
            # ax.annotate('峰值', xy=(time_values[-1], filtered_voltage[-1]), xytext=(0.6, 1.2),arrowprops=dict(facecolor='black', shrink=0.05))
            
            if voltage_values:  # 只有当电压值列表不为空时，才更新y轴下限
                min_voltage = min(voltage_values)  # 重新计算最小电压值
                ax.set_ylim(bottom=min_voltage*0.95)  # 使用更新后的最小电压值设置y轴下限
